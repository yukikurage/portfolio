...
titleBar =
  div [css "w-full h-auto px-5 bg-center bg-title-bg bg-cover relative before:bg-[white] before:opacity-0 before:absolute before:inset-0 pointer-events-auto overflow-hidden flex-shrink-0"]
  [ div [css "flex items-center justify-start relative w-full h-full"]
    [ img [css "h-12 w-auto mr-2", src "./images/icon512_black.png"]
